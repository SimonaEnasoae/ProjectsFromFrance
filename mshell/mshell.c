...
int main(int argc, char **argv) {
    ...
    assert(sigaction_wrapper(SIGINT, sigint_handler) != -1);   /* ctrl-c */
    assert(sigaction_wrapper(SIGTSTP, sigtstp_handler) != -1); /* ctrl-z */
    assert(sigaction_wrapper(SIGCHLD, sigchld_handler) != -1); /* Terminated or stopped child */
