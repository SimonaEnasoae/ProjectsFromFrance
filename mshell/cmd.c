...
void do_bg(char **argv) {
    ...
    if(job!=NULL){
      kill(-job->jb_pid, SIGCONT);  /* the "-" before the pid is used that we can also send signal to a process group */
      job->jb_state = BG;
    }
    return;
}
...
void do_stop(char **argv) {
  struct job_t *job = treat_argv(argv);
  if(job!=NULL){
    kill(-job->jb_pid, SIGTSTP); /* the "-" before the pid is used that we can also send signal to a process group */
    job->jb_state = ST;
  }
    return;
}
...
void do_kill(char **argv) {
  struct job_t *job = treat_argv(argv);
  if(job!=NULL){
    kill(-job->jb_pid, SIGKILL);  /* the "-" before the pid is used that we can also send signal to a process group */
...
void do_exit() {
    ...
    if (job != NULL){
        /* to exit the shell there are no stopped process*/
